...
def create_app(test_config=None):
    ...
    def get_categories():
        ...
        for category in categories:
            cat_data.append({"id": category.id, "type": category.type})
    ...
    def show_category(id):
        categories = Category.query.order_by(Category.type.asc()).all()
        entries = Entry.query.filter(Entry.category == id).order_by(
            Entry.date.desc()).all()
    ...
    def add_entry():
        if request.method == 'POST':
            ...
            try:
                ...
                if error:
                    db.session.rollback()
                    flash('ERROR: Entry ' + form.name.data +
    ...
    def update_entry(payload, id):
        ...
        category = Category.query.filter(
            Category.id == entry.category).one_or_none()
        ...
        try:
            ...
            if error:
                db.session.rollback()
                flash('ERROR: Entry ' + update.name.data +
    ...
    def logout():
        return redirect(
            'https://dev-mrose.us.auth0.com/v2/logout?client_id=kYWBov5tyiYk90E6osGWhtF5IKxCfKzN&returnTo=https://udacity-fsnd-tot.herokuapp.com/'
        )
